...
async function backupNow(sqliteDb, uploadsDir) {
  if (process.env.BACKUP_PAUSED === '1') return { skipped: 'paused via BACKUP_PAUSED=1' };
  const p = getPool();
...
async function restoreIfEmpty(sqliteDb, uploadsDir) {
  ...
  return restoreFromPostgres(sqliteDb, uploadsDir);
}

/** Force-restore Postgres snapshot into sqlite + uploads (admin-triggered, no empty check). */
async function restoreFromPostgres(sqliteDb, uploadsDir) {
  const p = getPool();
...
function scheduleBackup(sqliteDb, uploadsDir, delayMs = 30000) {
  if (process.env.BACKUP_PAUSED === '1') return;
...
module.exports = { isEnabled, getPool, ensureSchema, backupNow, restoreIfEmpty, restoreFromPostgres, getStatus, scheduleBackup };
